...
| --- | --- |
| `Schedule` | `schedule()` creates a row and subscribes before dispatching `requested` |
| `Receive` / `Notify` | `requested` event; queues idle work or marks an outstanding pass dirty |
| `Start` | `start` event in a microtask; the `read` effect captures missing metadata |
| `Complete` | `loaded` produces an `apply` effect; its outcome becomes a `completed` event |
| `Continue` | `continue` event: another read, or resolve initial loading and become idle |
| `Unschedule` | `removed` event followed by rejection/detachment effects; also abstracts collection |
...
The TypeScript transitions live in
[`RefreshSchedulerState.ts`](../src/RefreshSchedulerState.ts).
`transition(row, event)` mutates one row and returns effects, without invoking
external code. [`RefreshScheduler.ts`](../src/RefreshScheduler.ts) owns the active
rows and executes those effects. An async completion retains its original row;
removal invalidates that row before detaching it, and re-registration creates a
fresh object.
...
loading produces an apply effect, which can notify, remove the row, or throw.
The row remains `reading` through application and until its queued `completed`
event. This preserves the original drain's await boundary, including for empty
snapshots and synchronous errors. `completed` enters `after`, handles failure,
and emits a `continue` effect. That effect reads the row again *after* error or
